...
//	}
	...
	public List<T> loadByQuery(String sql, Object[] params, int pageNo, int pageSize) {
		int first = pageNo * pageSize;
		int max = pageSize;
	...
	public List loadsByNativeQueryWithParams(String sql, Object[] params) {
		Query query = entityManager.createNativeQuery(sql);
		for (int i = 0; params != null && i < params.length; i++) {
			query.setParameter(i + 1, params[i]);
		}
	...
	public List loadsByNativeQueryWithParams(String sql, Object[] params, int pageNo, int pageSize) {
		int first = pageNo * pageSize;
		int max = pageSize;
	...
	public T findBy(String columnName, String columnValue) {
		String clause = "t." + columnName + " = ?1";
		Object[] params = {columnValue};
		List<T> list = this.loadByClause(clause, params);
    ...
    public List<T> findByClause(String clause, Map<String, Object> params) {
        StringBuilder query = new StringBuilder();
        query.append("SELECT t FROM ").append(entityClass.getSimpleName()).append(" t ");

        if(clause != null && !clause.isEmpty()) {
            query.append("WHERE ").append(clause);
        }

        Query sql = entityManager.createQuery(query.toString());

        Iterator<Map.Entry<String, Object>> i = params.entrySet().iterator();
        while (i.hasNext()) {
            Map.Entry<String, Object> param = i.next();
            sql.setParameter(param.getKey(), param.getValue());
        }
